...
class Sgemm :
    ...
    def support_vector_regression(self):
        ...
        gamma = np.logspace(start=-1, stop=1, base=10, num=3, dtype=np.float32)
        ...
        for i in range(0, 4) :
            ...
            # training
            train_r2.append('%.3f' % svr.r2_score(
                x_test=self.X_train,
                y_test=self.y_train_list[i]))
            train_mse.append('%.3f' % svr.mean_squared_error(
                x_test=self.X_train,
                y_test=self.y_train_list[i]))
            # test
            test_r2.append('%.3f' % svr.r2_score(
                x_test=self.X_test,
                y_test=self.y_test_list[i]))
            test_mse.append('%.3f' % svr.mean_squared_error(
                x_test=self.X_test,
                y_test=self.y_test_list[i]))
...
if __name__ == '__main__':
    ...
    lls_results = sgemm.linear_least_squares()
