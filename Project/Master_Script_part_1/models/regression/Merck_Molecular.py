...
class Merck_molecular:
    ...
    def __init__(self, option=1 ,nov = 0):
        ...
        if nov == 0:
            # FEATURE REDUCTION BY DELETING COLUMNS
            idx = []
            for i in range(X.shape[1]):
                if (np.count_nonzero(X[:,i])/X.shape[0]) < 0.15:
                    idx.append(False)
                else:
                    idx.append(True)
            X = X[:,idx]
            ...
            pca.fit(X)
            print("%.2f of variance explained." % np.sum(pca.explained_variance_ratio_))
            X = pca.fit_transform(X)
        else:
            tsvd = sklearn.decomposition.TruncatedSVD(n_components = 400,random_state = 0)
            tsvd.fit(X)
            print("%.2f of variance explained." % np.sum(tsvd.explained_variance_ratio_))
        ...
        #change scoring strategy to average of the squared Pearson correlation
...
if __name__ == '__main__':
    ...
    for mm in mm_list:
        ...
        llr_results = call_with_timeout(180,mm.linear_least_squares)
